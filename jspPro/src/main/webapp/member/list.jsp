...
<div>
 ...
 <a href="<%=path%>/member/write.do">글쓰기</a>
 </c:if>
 <table>
   <thead>
     <tr>
     ...
     <c:choose>
       <c:when test="${not empty list }">
         <c:forEach var="dto" items="${list }">
           <tr>
             <td>${dto.seq }</td>
             <td class="title"><a href="<%=path%>/member/view.do?seq=${dto.seq }">${dto.title }</a></td>
             <td>${dto.writer }</td>
             <td>${dto.writedate }</td>
             <td>${dto.readed }</td>
           </tr>
   ...
   <tfoot>
     <tr>
       ...
       <div class="pagination">         
         <c:if test="${dto.prev }">
          <a href="<%=path%>/member/list.do?currentpage=${dto.start-1}">&lt;</a>
         </c:if>
         <c:forEach var="i" begin="${dto.start }" end="${dto.end }" step="1">
  		 ...
  		 <c:if test="${dto.next }">   
  		   <a href="<%=path%>/member/list.do?currentpage=${dto.end+1}">&gt;</a>
  		 </c:if>
	   </div>		 
	   </td>
     </tr>
